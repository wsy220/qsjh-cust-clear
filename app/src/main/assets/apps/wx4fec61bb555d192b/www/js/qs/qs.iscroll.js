...
(function($, window, document, undefined) {
	...
	$.initIscroll = function(oprtion) {
		...
		myScroll.downFlag = oprtion.downFlag | true;
		...
		if(!oprtion.page){
			...
			myScroll.page = oprtion.page
		}
		
		if(!oprtion.limit){
			myScroll.limit = 5;
		}else{
			myScroll.limit = oprtion.limit;
		}

//		myScroll.page = oprtion.page |1;
//		myScroll.limit = oprtion.limit?oprtion.limit:5

		//滚动时
		myScroll.on('scroll', function() {
			if(myScroll.loadingStep == 0 && !pullDownEl.class.match('flip|loading') && !pullUpEl.class.match('flip|loading')) {
				if(this.y > 20 && myScroll.downFlag) {
					myScroll.page = 1;
					//下拉刷新效果  
